...
router.route('/')
    .all(function (req, res, next) {
        ...
        options.title = req.t("index.title");
        next();
    })
    .post(function (req, res) {
        ...
        if (req.body.Invia) {
            ...
            getConnection(function (err, connection) {
                ...
                if (req.body.Invia == 'ELIMINA' || req.body.Invia == 'DELETE') {
                    ...
                    if (req.body.Invia == 'INSERISCI' || req.body.Invia == 'INSERT') {
                        ...
                        connection.query(sql, [req.session.user, sito, user, pwd], function (error) {
                            if (error)
                                ...
                                //creazione backup (selezione ultimo inserito)
                    ...
                    else if (req.body.Invia == 'MODIFICA' || req.body.Invia == 'EDIT') {
                        ...
                        connection.query(sql, [sito, user, pwd, id], function (error) {
                            if (error)
                                options.msg.errore = true;
                            else if (req.session.pin) { //aggiornamento backup
...
function setData(req, res) {
    getConnection(function (err, connection) {
        ...
        connection.query(sql, [req.session.user], function (error, results, fields) {
            ...
            options.dati = JSON.parse(JSON.stringify(results)); //eliminazione oggetto rowdata
            connection.release();
